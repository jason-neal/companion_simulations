...
def todcor(c1, c2, c12, pshift, sshift, images):
    ...
    g.write(images['science']['h']['targname'], images['science']['hjd'], 1, 1)

    for i, __ in enumerate(m):
        if error['p'][i] != '':
    ...
    for i, __ in enumerate(m):
        if error['p'][i] != '':
            g.write(orders[i], centroids[i, 0], centroids[i, 2], centroids[i, 1], \
                    centroids[i, 3])
        # #    f.close()
    g.close()
