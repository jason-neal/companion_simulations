...
# # # SPEC local norm 2 and assiciated are trying to use faster normalization techniques.


def spec_local_norm2(spectrum, splits=50, method="quadratic", plot=False, top=20):
    r"""Apply local normalization on Spectrum object.

    Split spectra into many chunks and get the average of top 5\% in each bin.
    """
    norm_spectrum = copy.copy(spectrum)
    flux_norm = local_normalization2(norm_spectrum.xaxis, norm_spectrum.flux, splits=splits, plot=plot, method=method, top=top)
    norm_spectrum.flux = flux_norm

    return norm_spectrum


def local_normalization2(wave, flux, splits=50, method="exponential", plot=False, top=20):
    r"""Local minimization for section of Phoenix spectra.
    ...
    norm_flux = continuum2(wave, flux, splits=splits, method=method, plot=plot, top=top)
...
def continuum2(wave, flux, splits=50, method='scalar', plot=False, top=20):
    """Fit continuum of flux.
    ...
    # print("wave points = ", wave_points, ", flux_points =", flux_points)
...
def chi2_model_norms2(wave, obs, models, method='scalar', splits=100, top=20):
    """Normalize the obs to the continuum of the models.
    ...
    obs_continuum = continuum2(wave, obs, splits=splits, method=method, top=top)

    # Try fit the apply_along_axis to get the continuum points only then polyfit n-D array.
    def axis_continuum(flux):
        """Continuum to apply along axis with predefined varaibles parameters."""
        return continuum2(wave, flux, splits=splits, method=method, top=top)
